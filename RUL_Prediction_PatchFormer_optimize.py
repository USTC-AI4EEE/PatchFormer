...
parser.add_argument('--model', default='PatchFormer_optimize', help='Model name.')
...
parser.add_argument('--max_epochs', type=int, default=5, help='max train epochs')
...
for start_point in args.start_point_list:
    ...
    while count<args.count:
        ...
        save_dir = sp_root_dir+'{}/'.format(str(model).split('\n')[0][:-1] + time.strftime(
                                                '_%Y-%m-%d %X', time.localtime()).replace(':', '-'))
        ...
        # 记录结果到张量板
        # logger = TensorBoardLogger(os.path.join(study_model_path,"lightning_logs"))  # logging results to a tensorboard
        # 将在trainer.fit过程中产生的训练结果输出到日志，你可以使用Lightning框架提供的回调功能
        # import logging
        # # 设置日志记录器
        # logger = logging.getLogger('lightning_logger')
        # logger.setLevel(logging.INFO)
        # # 创建一个文件处理器，将日志写入文件
        # file_handler = logging.FileHandler(stat_log_path)
        ...
        # # # 创建一个TensorBoardLogger实例
        # # tb_logger = TensorBoardLogger(save_dir=os.path.join(study_model_path,"logs/"), name='training_metrics')
        ...
        best_model = PatchFormerNetModel.load_from_checkpoint(best_model_path).to(device=device)
        ...
        print_log("实际运行的Epoch数量: {}  \n".format(trainer.current_epoch),log)
        print_log((
            '第{}次实验的结果: \nMAE:{:.4f}, RMSE:{:.4f}, r2:{:.4f}, RUL_real:{}, RUL_pred:{}, AE:{}, RE:{:.4f}  \n'.format(
                count,NMAE,NRMSE,r2,RUL_real,RUL_pred,AE,RE)), log)
        print_log("第{}次实验实际运行的Epoch数量: {}  \n".format(count,trainer.current_epoch),stat_log)
        print_log((
            '第{}次实验的结果: \nMAE:{:.4f}, RMSE:{:.4f}, r2:{:.4f}, RUL_real:{}, RUL_pred:{}, AE:{}, RE:{:.4f}  \n'.format(
                count,NMAE,NRMSE,r2,RUL_real,RUL_pred,AE,RE)), stat_log)
        # 保存模型
        trainer.save_checkpoint(
            log_path[:-4] + '_MAE_' + str(NMAE)[:6] + '_RMSE_' + str(NRMSE)[:6] + '_AE_' + str(AE) + '.ckpt')
        
        # --------------------------------------- 超参数调优 ---------------------------------
        '''
        使用 [optuna]( https://optuna.org/ ) 进行的超参数调整直接构建到 pytorch-forecasting 中。
        例如，我们可以使用该 optimize_hyperparameters()函数来优化 TFT 的超参数。
        '''
        #'''
        import pickle
        # from pytorch_forecasting.models.temporal_fusion_transformer.tuning import optimize_hyperparameters
        from ModelsModify.optimize import optimize_hyperparameters


        #  创建一个study，进行超参数优化； 优化方式：
        study = optimize_hyperparameters(
            PatchFormerNetModel,
            train_dataloaders=train_dataloader,
            val_dataloaders=val_dataloader,
            model_path=study_model_path,    # 设置保存模型的文件夹
            n_trials=200,                     # n_trials (int, optional): Number of hyperparameter trials to run. Defaults to 100.
            max_epochs=args.max_epochs,                   # max_epochs (int, optional): Maximum number of epochs to run training. Defaults to 20.
            enc_in = len(time_varying_known_reals),
        ...
        # 保存训练结果，ckpt格式，用pickle保存
        with open(study_model_path+".pkl", "wb") as fout:
        ...
        # 获取最佳模型路径  'lightning_logs\\default\\version_12\\checkpoints\\epoch=9-step=299.ckpt'

        with open(study_model_path+".pkl", 'rb') as f:
            study = pickle.load(f)
        print('study:', study)
        # 打印最佳模型
        print('best_trial:{}\n\n'.format(study.best_trial))
        # 打印最佳模型的关键参数
        print('study.best_trial  number:{} value:{} params:{}:\n'.format(study.best_trial.number, study.best_trial.value,
                                                                        study.best_trial.params))
        # 打印所有尝试的结果
        print('study.trials:{}\n'.format(study.trials))
        ...
        # 打印最佳模型
        print_log(('best_trial:{}\n\n'.format(study.best_trial)), log)
        # 打印最佳模型的关键参数
        print_log(('study.best_trial  number:{} value:{} params:{}:\n'.format(study.best_trial.number, study.best_trial.value,
                                                                            study.best_trial.params)), log)
        # 打印所有尝试的结果
        print_log(('study.trials:{}\n'.format(study.trials)), log)

        # ---- 获取study的最优模型保存结果---
        # 获取study最优模型的文件夹
        study_best_model_dir = os.path.join(study_model_path, 'trial_{}'.format(study.best_trial.number))
        # 获取study最优模型的路径
        study_best_model_path=os.path.join(study_best_model_dir, os.listdir(study_best_model_dir)[0])
        # 加载最佳模型参数
        best_study_model = PatchFormerNetModel.load_from_checkpoint(study_best_model_path)

        predictions = best_study_model.predict(test_dataloader,batch_size=256)
        predictions = predictions.detach().cpu().numpy().reshape(-1)
        actuals_df = df_all.loc[df_all['Cycle']>=start_point,['Cycle','target']]
        actuals = actuals_df['target'].values
        y_true = actuals*args.Rated_Capacity
        ...
        f=open(study_model_path+'.txt','w',encoding='UTF-8')
        #  打印实验结果
        print_log((
            '第{}次实验的最佳结果: \nMAE:{:.4f}, RMSE:{:.4f}, r2:{:.4f}, RUL_real:{}, RUL_pred:{}, AE:{}, RE:{:.4f}  \n'.format(
                count,NMAE,NRMSE,r2,RUL_real,RUL_pred,AE,RE)), f)
        print_log(('study_best_model_path:{}'.format(study_best_model_path)),f)
        copyfile(study_best_model_path,log_path[:-4]+'_Best_Study_MAE_'+ str(NMAE)[:6] + '_RMSE_' + str(NRMSE)[:6] + '_AE_' + str(AE) + '.ckpt')
    ...
    setting = '{}_sl{}_ll{}_pl{}_bs{}_ct{}_tn{}_me{}'.format(
            ...
            args.batch_size,
            args.count,
            int(args.test_name[-2:]),
            args.max_epochs)
    print_log('args setting : {}\n'.format(setting),stat_log)
    stat_log.close()
       
results = dict()
results['SP300'] = all_pred_data_list[0]
results['SP400'] = all_pred_data_list[1]
results['SP500'] = all_pred_data_list[2]
