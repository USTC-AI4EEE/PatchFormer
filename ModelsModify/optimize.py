...
def optimize_hyperparameters(
    ...
    **kwargs,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        elif 'PatchFormerNetModel' in Model.__name__:
            model = Model.from_dataset(
                train_dataloaders.dataset,
                seq_len=max_encoder_length,
                enc_in=enc_in,
                pred_len=max_prediction_length,
                patch_len=trial.suggest_categorical(name="patch_len",choices=[2,4,8]),
                dropout=trial.suggest_uniform(name="dropout",low=0.01,high=0.2),
                factor=trial.suggest_int(name="factor", low=1, high=5, step=1),
                d_model=trial.suggest_int(name="d_model",low=16,high=256, step=16),
                output_attention=False,
                n_heads=trial.suggest_int(name="n_heads",low=1,high=8,step=1),
                activation='gelu',
                e_layers=trial.suggest_int(name="e_layers", low=1, high=4, step = 1),
                loss=SMAPE(),
            )
        else:
            pass
