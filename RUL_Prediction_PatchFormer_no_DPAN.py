...
parser.add_argument('--model', default='PatchFormer_no_DPAN', help='Model name.')
...
for start_point in args.start_point_list:
    ...
    while count<args.count:
        ...
        # --------------------------------------step 2: 网络构建--------------------------------------------------------
        # ----------------------------------------- 创建基线模型 ----------------------------------------------
        from ModelsModify.PatchFormer_no_DPAN import PatchFormer_no_DPANNetModel
        model = PatchFormer_no_DPANNetModel.from_dataset(
            training,
        ...
        best_model = PatchFormer_no_DPANNetModel.load_from_checkpoint(best_model_path).to(device=device)
